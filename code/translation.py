    ...
    with open(path, 'r', encoding='utf-8') as f:
        for line in f.readlines():
            ...
            if tgt_add_bos:
                ...
                cn.append(list(jieba.cut(line[1])))
...
class DataProcessor(object):
    def __init__(self, args):
        ...
        if not os.path.exists(cached_en_tokenizer) or not os.path.exists(cached_cn_tokenizer):
            en_sents, cn_sents = load_file(args.data_dir + "cmn.txt")
    ...
    def get_train_examples(self, args):
        return self._create_examples(os.path.join(args.data_dir, "cmn.txt"), "cmn", args)
