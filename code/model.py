...
class Decoder(nn.Module):
    ...
    def create_mask(self, x_len, y_len):
        ...
        x_mask = (torch.arange(max_x_len.item())[None, :] < x_len.long()[:, None]).float()  # [batch,max_x_len]
        y_mask = (torch.arange(max_y_len.item())[None, :] < y_len.long()[:, None]).float()  # [batch,max_y_len]
    ...
    def beam_search(self, x, x_lengths, y, EOS_id, topk=5, max_length=100):
        ...
        while len(completed_hypotheses) < topk and t < max_length:
            ...
            for prev_hyp_id, hyp_word_id, cand_new_hyp_score in zip(prev_hyp_ids, hyp_word_ids, top_cand_hyp_scores):
                ...
                new_hyp_sent = hypotheses[int(prev_hyp_id)] + [int(hyp_word_id)]
